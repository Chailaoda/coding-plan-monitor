...
STEPFUN_TARGET_HOSTS = (".stepfun.com", "platform.stepfun.com")


def load_from_360_stepfun(db_path: Optional[str] = None) -> Tuple[str, str, Optional[str]]:
    """
    从 360 浏览器读取 stepfun.com 的 Cookie（用于阶跃星辰用量查询）。

    Returns:
        (cookie_header, oasis_token, oasis_webid_or_None)
    ...
    if not os.path.exists(db_path):
        raise CookieLoadError(
            f"360 浏览器 Cookie 数据库不存在: {db_path}\n"
            "请确认浏览器已安装并至少登录过 platform.stepfun.com 一次。"
        )
    ...
    try:
        ...
        conn = sqlite3.connect(tmp)
        cur = conn.cursor()
        ph = ",".join("?" for _ in STEPFUN_TARGET_HOSTS)
        cur.execute(
            f"SELECT host_key, name, encrypted_value FROM cookies WHERE host_key IN ({ph})",
            STEPFUN_TARGET_HOSTS,
        )
        rows = cur.fetchall()
        conn.close()
    except sqlite3.Error as e:
    ...
    if not rows:
        raise CookieLoadError(
            "Cookie 数据库中未找到 stepfun.com 的记录，请先在 360 浏览器中登录阶跃星辰平台。"
        )
    ...
    if "Oasis-Token" not in cookies:
        raise CookieLoadError(
            "未找到 Oasis-Token，请先在 360 浏览器中登录 platform.stepfun.com。"
        )

    oasis_token = cookies["Oasis-Token"]
    oasis_webid = cookies.get("Oasis-Webid")
    cookie_header = "; ".join(f"{k}={v}" for k, v in cookies.items() if v)
    return cookie_header, oasis_token, oasis_webid
